...
function StarRating({
  ...
  const handleClick = () => {
    if (type === "productsPage" && onRatingSelect) {
  ...
  const stars = Array.from({ length: maxStars }, (_, index) => {
    ...
    return (
      <span key={index} className="inline-block relative">
        <svg
          className="w-5 h-5 text-gray-300 transition-all duration-200"
          fill="currentColor"
          stroke={isHovering ? "#C7D2FE" : "currentColor"}
          strokeWidth="1.5"
          viewBox="-1 -1 20 20"
        >
        ...
        {isFullStar && (
          <svg
            className="w-5 h-5 text-yellow-400 absolute top-0 left-0 transition-all duration-200"
            fill={isSelected ? "#EAB308" : "currentColor"}
            stroke={isHovering ? "#C7D2FE" : "currentColor"}
            strokeWidth="1.5"
            viewBox="-1 -1 20 20"
          >
  ...
  return (
    <div
      className={`flex items-center gap-[9px] ${type === "productDetail" ? "pt-[16px] pb-[34px]" : "pb-[15px] pt-[8px]"} ${type === "productsPage" ? "cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors duration-200" : ""} `}
      onMouseEnter={() => setIsHovering(true)}
      onMouseLeave={() => setIsHovering(false)}
      onClick={handleClick}
    >
      {type !== "reviews" && type !== "productsPage" && (
      ...
      <div
        className="flex gap-[8px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:rounded-[2px]"
        tabIndex={0}
      >
        {stars}
      </div>
